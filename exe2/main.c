...
void btn_2_task(void *p) {
  ...
  while (true) {
    if (!gpio_get(BTN_PIN_G)) { //botao pressionado -> 0
      while (!gpio_get(BTN_PIN_G)) { //debounce botao
        vTaskDelay(pdMS_TO_TICKS(1)); //bloqueia ESSA task e continua rodando outros.
      }
...
int main() {
  ...
  xTaskCreate(led_2_task, "BTN_Task 2", 256, NULL, 1, NULL);
